...
By default, the greeting page's background is a random photo from [picsum.photos](https://picsum.photos/) (free, Unsplash-sourced -- free for commercial and non-commercial use, no attribution required). If you'd rather use your own photos, drop them into `assets/background/<cwa>/` (one folder per NWS County Warning Area code, e.g. `assets/background/bmx/` for Birmingham, AL) -- optionally split further into phenomenon subfolders (`assets/background/bmx/tornado/`, `.../snow/`, `.../clearday/`, etc.) so the photo actually matches the active alert or current condition -- and one is picked at random on each load; it falls back to the plain CWA folder, then to picsum.photos, if nothing more specific is available for the current location. See [assets/background/README.md](./assets/background/README.md) for the full phenomenon list and priority order, plus how to point Docker at a photo folder elsewhere on the host (or a network share) instead of the repo's own folder.
